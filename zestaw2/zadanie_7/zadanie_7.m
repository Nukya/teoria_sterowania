%% Zadanie 7 - Sterowanie z minimalnym wydatkiem (L1)
% Dodatkowe ograniczenia:
%   - narzucone wartości u_0 = u_{N-1} = 0,
%   - jednostronne ograniczenie zmian sterowania: u_{k+1} - u_k <= 0.02.
...
%% Jednostronne ograniczenie różnic: u_{k+1} - u_k <= 0.02
D = zeros(N-1,N);
for k = 1:N-1
    D(k,k)   = -1;
    D(k,k+1) =  1;
...
title('Zadanie 7: sterowanie dyskretne $u(k)$');
grid on;
...
title('Zadanie 7: przebiegi stanu w czasie ciągłym');
grid on;

subplot(3,1,3);
plot(x_c(:,1), x_c(:,2), 'LineWidth', 1.5);
xlabel('$x_1$'); ylabel('$x_2$');
title('Zadanie 7: trajektoria w przestrzeni stanu');
grid on;
